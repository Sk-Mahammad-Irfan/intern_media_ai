...
function appendUserMessage(prompt) {
  ...
  userWrapper.innerHTML = `
      ...
      <i 
        class="bi bi-clipboard-fill text-secondary mt-1" 
...
function appendGeneratingVideoMessage(resolution) {
...
function appendGeneratedVideo(videoUrl) {
  ...
  const resolution =
    document.getElementById("resolutionSelect").value || "540p";
  const aspect_ratio =
  ...
  // Use actual aspect ratio if possible
...
async function generateVideo() {
  ...
  const aspect_ratio = document.getElementById("aspectRatioSelect").value;
  ...
  appendGeneratingVideoMessage(resolution);
...
function populateModelOptions(modelId) {
  ...
  // Clear existing
  resolutionSelect.innerHTML = `<option value="" selected>Select resolution</option>`;
  aspectRatioSelect.innerHTML = `<option value="" selected>Select aspect ratio</option>`;

  if (!config) {
    console.warn("No config found for model:", modelId);
    return;
  }

  // ✅ Fix: Iterate over keys of resolution object
  ...
  if (config.resolutions["720p"] !== undefined) {
