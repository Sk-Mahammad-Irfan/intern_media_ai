document.addEventListener("DOMContentLoaded", async () => {
  try {
    ...
    // X-axis labels are just position numbers starting from 1
    const labels = sortedData.map((_, index) => (index + 1).toString());
          ...
          tooltip: {
            ...
            callbacks: {
              title: (context) => {
                ...
                const modelName = sortedData[index].model;
                return modelName; // This is the title of the tooltip
              },
              label: (context) => {
