...
function updateModelStatus(isDefault = false) {
  ...
  statusEl.innerHTML = `You can select one of the models from the featured models. Currently it's set to <strong>${modelName}${
    isDefault ? " (default)" : ""
  }</strong>.`;
}
...
const defaultCard = document.querySelector(
  `.model-card[data-model="${selectedModel}"]`
);
if (defaultCard) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (userData?.role === "1") {
    adminLink.classList.remove("d-none"); // show only if admin
  }
});
...
document.getElementById("submitPrompt").addEventListener("click", function () {
  ...
  // Redirect
  window.location.href = `${page}?id=${selectedModel}&prompt=${encodeURIComponent(
    prompt
  )}`;
});
